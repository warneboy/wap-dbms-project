...
app.post("/login", (req, res) => {
  ...
  db.query("SELECT * FROM customers WHERE email=?", [email], async (err, rows)=>{
    ...
    if(rows.length){
      ...
      const token = jwt.sign({ id: rows[0].id, role:"customer" }, JWT_SECRET, { expiresIn:"1d" });
    ...
    db.query("SELECT * FROM shopkeepers WHERE email=?", [email], async (err, rows)=>{
      ...
      const token = jwt.sign({ id: rows[0].id, role:"shopkeeper" }, JWT_SECRET, { expiresIn:"1d" });
      res.json({ message:"Login successful", role:"shopkeeper", token });
    });
  });
});
